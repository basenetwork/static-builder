...
var _dataDir = __dirname + '/../../node-server/data/';
...
function compile(filename) {
    ...
    return cont.split("\n").map(function(line) {
        ...
        var m = line.match(/^\s*\/\/\s*include\s*(\S+)\s*$/) || line.match(/^@import\s*"(\S+)";\s*$/);
        if (m) {
            var filename = m[1];
            if(fs.existsSync(stdDir + filename)) return compile(stdDir + filename);
            return compile(dir + filename);
        }
        return line;
    }).join("\n");
}
